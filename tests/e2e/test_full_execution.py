    ...
    "graph": {
        "nodes": [
            ...
            },
            {
                ...
                "pos": [200, 0],
                "properties": {
                    "message": "👋 Hello from E2E Test! System is Operational.",
                    "level": "INFO",
                },
                "custom": {},
            },
...
async def run_e2e_test():
    print(f"🚀 Starting E2E Test against {ORCHESTRATOR_URL}")
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"   ❌ Orchestrator Unhealthy: {resp.status_code}")
                return
        except Exception as e:
            print(f"   ❌ Failed to connect: {e}")
            print("   ⚠️  Please ensure 'launch.bat' is running!")
            return

        # 2. Check Robot Status
        print("\n2️⃣  Checking Robot Fleet...")
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(payload, list):
                    ...
                    print(f"   ❌ Unexpected response format: {payload}")
                    return
                # Ensure it's a list
                if not isinstance(robots, list):
                    print(f"   ❌ 'data' is not a list: {type(robots)} - {robots}")
                    return

                online_robots = [r for r in robots if r["status"] == "online"]
                print(f"   ℹ️  Found {len(robots)} robots, {len(online_robots)} ONLINE")
                if not online_robots:
                    print("   ⚠️  No robots online. Job will stay pending.")
                else:
                    print(f"   ✅ Targeted Robot: {online_robots[0]['name']}")
            elif resp.status_code == 401:
                print("   ❌ Authentication Failed (401). Check API_SECRET.")
                return
            else:
        ...
        try:
            ...
            if create_resp.status_code in (200, 201):
                ...
                print(
                    f"   ❌ Failed to create workflow: {create_resp.status_code} {create_resp.text}"
                )
                return
