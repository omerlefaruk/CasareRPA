-- Migration: 011_robot_logs
-- Description: Robot log streaming with 30-day retention and partitioning
-- Consolidated from:
...
-- Index for querying by robot and time
...
-- Index for full-text search on message
...
-- COMMENTS
-- =============================================================================
COMMENT ON TABLE robot_logs IS 'Real-time log entries from robots, partitioned by month';
COMMENT ON TABLE robot_logs_cleanup_history IS 'Audit trail for log cleanup operations';
COMMENT ON FUNCTION create_robot_logs_partition IS 'Create a new monthly partition';
COMMENT ON FUNCTION drop_old_robot_logs_partitions IS 'Drop partitions older than retention period';
COMMENT ON FUNCTION query_robot_logs IS 'Query logs with filtering by tenant, robot, level, time, etc.';
