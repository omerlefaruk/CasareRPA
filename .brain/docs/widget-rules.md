...
### Conflict Resolution
`@node_schema` auto-generates widgets BEFORE manual code runs. Use `_replace_widget()`:
...
| None | ∅ | `get_type_color("none")` | Null values |

### Integration
Auto-generated by `@node_schema` via `_add_variable_aware_text_input()` in base_visual_node.py.
