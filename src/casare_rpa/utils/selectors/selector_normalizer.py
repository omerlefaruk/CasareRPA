...
def normalize_selector(selector: str) -> str:
    ...
    if selector.startswith('/') and not selector.startswith('//'):
        ...
        logger.debug(f"Adding xpath= prefix to absolute path: {selector} -> {normalized}")
        return normalized
    
    # 7. Check if it's a CSS attribute selector (starts with [ but no XPath functions)
    if selector.startswith('[') and not any(func in selector for func in ['contains(', 'text()', 'normalize-space(']):
    ...
    if '@' in selector or any(func in selector for func in ['contains(', 'text()', 'normalize-space(']):
        # Looks like XPath but doesn't start with / - add // prefix
        if not selector.startswith('/'):
            normalized = f'//{selector}'
            logger.debug(f"Adding // prefix to XPath expression: {selector} -> {normalized}")
            return normalized
...
def detect_selector_type(selector: str) -> str:
    ...
    if selector.startswith('text='):
        return 'text'
    if selector.startswith('//') or (selector.startswith('/') and not selector.startswith('/[')):
        return 'xpath'
