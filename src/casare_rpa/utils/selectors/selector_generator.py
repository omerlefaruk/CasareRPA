...
class ElementFingerprint:
    ...
    def get_fallback_selectors(self) -> List[SelectorStrategy]:
        ...
        sorted_selectors = sorted(
            self.selectors, key=lambda s: (-s.score, s.failure_count)
        )
...
class SmartSelectorGenerator:
    ...
    def generate_selectors(cls, element_data: Dict) -> ElementFingerprint:
        ...
        if xpath := selectors_data.get("xpath"):
            ...
            score = cls._score_selector(
                SelectorType.XPATH, optimized_xpath, element_data
            )
            strategies.append(
                SelectorStrategy(
                    selector_type=SelectorType.XPATH, value=optimized_xpath, score=score
                )
            )
        ...
        if aria_xpath := selectors_data.get("aria"):
            ...
            strategies.append(
                SelectorStrategy(
                    selector_type=SelectorType.ARIA, value=aria_xpath, score=score
                )
            )
        ...
        if aria_label := element_data.get("attributes", {}).get("aria-label"):
            ...
            strategies.append(
                SelectorStrategy(
                    selector_type=SelectorType.ARIA, value=xpath_aria, score=score
                )
            )
        ...
        if css := selectors_data.get("css"):
            ...
            strategies.append(
                SelectorStrategy(selector_type=SelectorType.CSS, value=css, score=score)
            )
        ...
        if text := selectors_data.get("text"):
            ...
            strategies.append(
                SelectorStrategy(
                    selector_type=SelectorType.TEXT, value=xpath_text, score=score
                )
            )
    ...
    def validate_selector_uniqueness(
        selector_value: str, selector_type: SelectorType
    ) -> bool:
        """
    ...
    @staticmethod
    def calculate_element_similarity(
        fp1: ElementFingerprint, fp2: ElementFingerprint
    ) -> float:
        """
        ...
        if fp1.class_names and fp2.class_names:
            ...
            if common_classes:
                score += 15 * (
                    len(common_classes)
                    / max(len(fp1.class_names), len(fp2.class_names))
                )
            weights += 15
        ...
        if fp1.attributes and fp2.attributes:
            ...
            if common_attrs:
                score += 10 * (
                    len(common_attrs) / max(len(fp1.attributes), len(fp2.attributes))
                )
            weights += 10
