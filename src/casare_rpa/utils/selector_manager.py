...
class SelectorManager:
    ...
    async def activate_selector_mode(self, recording: bool = False, 
        ...
        # Activate selector mode in browser
        await self._active_page.evaluate(
            f"window.__casareRPA.selector.activate({str(recording).lower()})"
        )
    ...
    async def _validate_selectors(self, fingerprint: ElementFingerprint):
        ...
        for selector_strategy in fingerprint.selectors:
            try:
                # Test selector
                if selector_strategy.selector_type.value in ['xpath', 'aria', 'data_attr', 'text']:
                    # XPath-based selectors
                    result = await self._active_page.evaluate(f"""
                        (() => {{
                            const start = performance.now();
                            const nodes = document.evaluate(
                                `{selector_strategy.value}`,
                                document,
                                null,
                    ...
                    result = await self._active_page.evaluate(f"""
                        (() => {{
                            const start = performance.now();
                            const elements = document.querySelectorAll(`{selector_strategy.value}`);
                            const time = performance.now() - start;
        ...
        try:
            if selector_type in ['xpath', 'aria', 'data_attr', 'text']:
                ...
                result = await self._active_page.evaluate(f"""
                    (() => {{
                        const start = performance.now();
                        const elements = document.querySelectorAll(`{selector_value}`);
                        const time = performance.now() - start;
                        return {{ 
    ...
    async def highlight_elements(self, selector_value: str, selector_type: str = 'xpath'):
        ...
        try:
            if selector_type in ['xpath', 'aria', 'data_attr', 'text']:
                await self._active_page.evaluate(f"""
                    (() => {{
