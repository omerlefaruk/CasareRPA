...
class OrchestratorWindow(QMainWindow):
    def __init__(self):
        ...
        header.setStyleSheet(
            f"font-size: 24px; font-weight: bold; margin-bottom: 10px; color: {THEME.text_primary};"
        )
        ...
        self.timer.timeout.connect(
            lambda: asyncio.get_event_loop().create_task(self.refresh_data())
        )
    ...
    def setup_jobs_tab(self):
        ...
        self.jobs_table.setHorizontalHeaderLabels(
            ["Job ID", "Robot", "Workflow", "Status", "Time"]
        )
    ...
    def update_robots_table(self, robots):
        ...
        for i, robot in enumerate(robots):
            ...
            dispatch_btn.clicked.connect(
                lambda checked, r=robot: self.dispatch_workflow(r)
            )
    ...
    def update_jobs_table(self, jobs):
        ...
        for i, job in enumerate(jobs):
            ...
            workflow_preview = (
                job.get("workflow", "")[:50] + "..."
                if len(job.get("workflow", "")) > 50
                else job.get("workflow", "")
            )
            self.jobs_table.setItem(i, 2, QTableWidgetItem(workflow_preview))
            self.jobs_table.setItem(
                i, 3, QTableWidgetItem(job.get("status", "unknown"))
            )
            self.jobs_table.setItem(
                i, 4, QTableWidgetItem(job.get("created_at", "N/A"))
            )
    ...
    def dispatch_workflow(self, robot):
        ...
        try:
            ...
            asyncio.get_event_loop().create_task(
                self._dispatch_job(robot_id, workflow_json, Path(file_path).name)
            )
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self,
                    "Success",
                    f"Workflow '{filename}' dispatched to robot {robot_id}",
                )
