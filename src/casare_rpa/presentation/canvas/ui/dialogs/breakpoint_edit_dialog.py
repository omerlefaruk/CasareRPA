...
Features:
- Breakpoint type selector (Regular, Conditional, Hit-Count, Log Point)
- Condition expression input with syntax highlighting
- Hit count target input for hit-count breakpoints
- Log message template input for log points
...
class BreakpointEditDialog(QDialog):
    ...
    def __init__(
        ...
        self.setWindowTitle("Edit Breakpoint" if breakpoint else "Add Breakpoint")
        self.setModal(True)

        apply_dialog_style(self, DialogSize.MD, resizable=False)
    ...
    def _setup_ui(self) -> None:
        ...
        # Header
        header = QLabel("Breakpoint Configuration")
        header.setStyleSheet(DialogStyles.header())
        layout.addWidget(header)

        # Node info
        node_label = QLabel(f"Node: {self._node_id}")
        node_label.setStyleSheet(DialogStyles.info_label())
        ...
        regular_info.setStyleSheet(DialogStyles.info_label())
    ...
    def _create_conditional_page(self) -> QWidget:
        ...
        info.setStyleSheet(DialogStyles.info_label())
        ...
        test_btn.setStyleSheet(DialogStyles.button_secondary())
    ...
    def _show_test_result(self, message: str, status: str) -> None:
        ...
        style_map = {
            "success": DialogStyles.success_label(),
            "error": DialogStyles.error_label(),
            "warning": DialogStyles.warning_label(),
        }
        self._test_result.setStyleSheet(style_map.get(status, ""))
