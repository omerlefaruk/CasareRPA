...
Usage (in app.py, before QApplication creates widgets):
    from casare_rpa.presentation.canvas.theme.font_loader import ensure_font_registered
...
def _register_font_family(family_name: str, font_files: tuple[str, ...], fonts_dir: Path) -> bool:
    """
...
def ensure_font_registered() -> None:
    ...
    if not fonts_dir.exists():
        logger.debug(f"Fonts directory not found: {fonts_dir} - using system font fallbacks")
        _font_registered = True
        return

    # Try to register Geist Sans
    geist_sans_ok = _register_font_family(GEIST_SANS_FAMILY, GEIST_SANS_FILES, fonts_dir)

    # Try to register Geist Mono
    geist_mono_ok = _register_font_family(GEIST_MONO_FAMILY, GEIST_MONO_FILES, fonts_dir)
