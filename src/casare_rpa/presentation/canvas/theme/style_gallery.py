...
Usage:
    from casare_rpa.presentation.canvas.theme.style_gallery import show_style_gallery_v2
...
def _create_icons_section(parent: QWidget) -> QGroupBox:
    ...
    for size in sizes:
        ...
        for icon_name, label in icons_to_show:
            ...
            name_label.setStyleSheet(
                f"font-size: {TOKENS_V2.typography.caption}px; color: {THEME_V2.text_secondary};"
            )
    ...
    state_label.setStyleSheet(
        f"font-weight: {TOKENS_V2.typography.weight_medium}; margin-top: {TOKENS_V2.spacing.md}px;"
    )
...
def show_style_gallery_v2(parent=None) -> QDialog:
    ...
    content_layout.setContentsMargins(
        TOKENS_V2.spacing.xl, TOKENS_V2.spacing.xl, TOKENS_V2.spacing.xl, TOKENS_V2.spacing.xl
    )
    ...
    subtitle = QLabel(
        "Epic 1.3: Global QSS v2 generated from tokens (no per-widget setStyleSheet required)",
        content,
    )
