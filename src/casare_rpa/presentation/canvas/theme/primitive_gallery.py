...
Usage:
    from casare_rpa.presentation.canvas.theme.primitive_gallery import (
...
class PrimitiveGallery(QDialog):
    ...
    def _add_button_sizes_section(self, layout: QVBoxLayout) -> None:
        ...
        for size in ["sm", "md", "lg"]:
            row = self._create_row(
                size.upper(), PushButton(text=size.upper(), size=size, variant="primary")
            )
    ...
    def _add_text_input_section(self, layout: QVBoxLayout) -> None:
        ...
        row3 = self._create_row(
            "Clearable:", TextInput(placeholder="Type to see clear button", clearable=True)
        )
    ...
    def _add_search_input_section(self, layout: QVBoxLayout) -> None:
        ...
        row2 = self._create_row(
            "With value:", SearchInput(value="existing query", placeholder="Search...")
        )
        layout.addWidget(row2)
    ...
    def _add_double_spin_box_section(self, layout: QVBoxLayout) -> None:
        ...
        row2 = self._create_row(
            "With prefix:", DoubleSpinBox(min=0.0, max=1000.0, value=99.99, prefix="$ ", decimals=2)
        )
        layout.addWidget(row2)

        row3 = self._create_row(
            "With suffix:", DoubleSpinBox(min=0.1, max=5.0, value=1.0, suffix="x", step=0.1)
        )
        layout.addWidget(row3)
    ...
    def _add_select_section(self, layout: QVBoxLayout) -> None:
        ...
        row3 = self._create_row(
            "Clearable:", Select(items=items, placeholder="Select...", clearable=True, value="2")
        )
        layout.addWidget(row3)
    ...
    def _add_slider_section(self, layout: QVBoxLayout) -> None:
        ...
        for size in ["sm", "md", "lg"]:
            row = self._create_row(
                f"{size.upper()}:", Slider(min=0, max=100, value=50, show_value=True, size=size)
            )
            layout.addWidget(row)
        ...
        if file_path:
            if image.save(file_path):
                ...
                QMessageBox.information(
                    self, "Export Complete", f"Screenshot saved to:\n{file_path}"
                )
            else:
                logger.error(f"Failed to save screenshot to: {file_path}")
                QMessageBox.warning(
                    self, "Export Failed", f"Failed to save screenshot to:\n{file_path}"
                )
