"""
CasareRPA Configuration File Loader.
...
class FileConfigurationError(Exception):
    """Raised when configuration file loading or validation fails."""
...
class ConfigFileLoader:
    ...
    def load(
        ...
        Raises:
            FileConfigurationError: If file doesn't exist and required=True
        """
        path = Path(path)
        ...
        if not path.exists():
            if required:
                raise FileConfigurationError(f"Configuration file not found: {path}")
        ...
        try:
            if format == "yaml":
                ...
                raise FileConfigurationError(f"Unsupported config format: {format}")

            self._cache[cache_key] = config
            return deepcopy(config)

        except Exception as e:
            raise FileConfigurationError(f"Failed to load {path}: {e}") from e
    ...
    def validate(self, config: Dict[str, Any], schema: ConfigSchema) -> Dict[str, Any]:
        ...
        Raises:
            FileConfigurationError: If validation fails
        """
        errors = []
    ...
    def _load_from_env(self) -> Dict[str, Any]:
        ...
        for key, value in os.environ.items():
            if key.startswith(self.env_prefix):
                ...
                # Handle nested keys (e.g., CASARE_DATABASE__HOST -> database.host)
...
def load_config_file(path: Union[str, Path], required: bool = True) -> Dict[str, Any]:
    """
...
def load_config_file_with_env(
    ...
    """
    loader = ConfigFileLoader(env_prefix=env_prefix)
