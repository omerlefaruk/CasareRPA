...
from ...utils.selectors.selector_manager import SelectorManager
from ...utils.selectors.selector_generator import ElementFingerprint
...
class SelectorIntegration(QObject):
    ...
    async def start_picking(self, target_node=None, target_property: str = "selector"):
        ...
        await self.selector_manager.activate_selector_mode(
            recording=False,
            on_element_selected=self._handle_element_selected
        )
    ...
    async def start_recording(self):
        ...
        await self.selector_manager.activate_selector_mode(
            recording=True,
            on_recording_complete=self._handle_recording_complete
        )
    ...
    def _handle_element_selected(self, fingerprint: ElementFingerprint):
        ...
        def test_callback(selector_value: str, selector_type: str) -> Dict[str, Any]:
            ...
            if loop.is_running():
                ...
                try:
                    return loop.run_until_complete(asyncio.wait_for(future, timeout=2.0))
    ...
    def _update_node_property(self, selector_value: str):
        ...
        try:
            ...
            if widget:
                ...
                logger.info(f"Updated {self._target_node.name()}.{self._target_property} = {selector_value}")
            else:
