...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QListWidget, QListWidgetItem, QWidget, QSplitter,
...
from ...utils.selectors.selector_generator import ElementFingerprint, SelectorStrategy, SelectorType
...
class SelectorDialog(QDialog):
    ...
    def __init__(self, fingerprint: ElementFingerprint, 
    ...
    def _create_details_panel(self) -> QWidget:
        ...
        self.test_results.setStyleSheet("padding: 8px; background: #2d2d2d; border-radius: 4px; color: #e0e0e0;")
    ...
    def _on_selector_changed(self, current: QListWidgetItem, previous: QListWidgetItem):
        ...
        score_color = "#4caf50" if strategy.score >= 80 else "#ff9800" if strategy.score >= 60 else "#f44336"
    ...
    def _on_test_selector(self):
        ...
        if not self.selected_strategy or not self.test_callback:
            ...
            self.test_results.setStyleSheet("padding: 8px; background: #fff3cd; border-radius: 4px;")
            return
    ...
    def _do_test(self, selector_value: str, selector_type: str):
        ...
        try:
            ...
            if result.get('success'):
                ...
                if count == 0:
                    self.test_results.setText(f"❌ No elements found")
                    self.test_results.setStyleSheet("padding: 8px; background: #3d1e1e; color: #ef4444; border: 1px solid #7f1d1d; border-radius: 4px;")
                elif count == 1:
                    self.test_results.setText(f"✓ Found exactly 1 element\nExecution time: {time_ms:.2f}ms")
                    self.test_results.setStyleSheet("padding: 8px; background: #1e3d2e; color: #10b981; border: 1px solid #065f46; border-radius: 4px;")
                else:
                    self.test_results.setText(f"⚠ Found {count} elements (not unique)\nExecution time: {time_ms:.2f}ms")
                    self.test_results.setStyleSheet("padding: 8px; background: #3d3520; color: #fbbf24; border: 1px solid #78350f; border-radius: 4px;")
        ...
        if self.target_node and self.target_property:
            try:
                ...
                if widget:
                    ...
                    logger.warning(f"Widget '{self.target_property}' not found on node {self.target_node.name()}")
            except Exception as e:
    ...
    def get_selected_selector(self) -> tuple[str, str]:
        ...
        return (
            self.selector_edit.toPlainText().strip(),
