...
class CasareRPAApp:
    ...
    def _connect_signals(self) -> None:
        ...
        if debug_toolbar:
            ...
            debug_toolbar.clear_breakpoints_requested.connect(
        ...
        self._main_window.action_pick_selector.triggered.connect(
            self._on_start_selector_picking
        )
        self._main_window.action_record_workflow.triggered.connect(
            self._on_toggle_recording
        )

        # File/Scenario operations
        self._main_window.action_save_to_scenario.triggered.connect(
        ...
        if hasattr(graph, "node_selection_changed"):
            graph.node_selection_changed.connect(
                self._on_selection_changed_for_properties
            )
    ...
    def _on_duplicate_nodes(self) -> None:
        ...
        logger.debug(
            f"Duplicated {len(pasted_nodes)} nodes with center at ({scene_pos.x()}, {scene_pos.y()})"
        )
    ...
    def _cleanup_orphaned_pipes(self) -> None:
        ...
        try:
            ...
            for item in scene.items():
                ...
                if "Pipe" in class_name and class_name != "LivePipeItem":
                    ...
                    if input_port is None or output_port is None:
                        should_remove = True
                        logger.debug("Orphaned pipe: missing port(s)")
                    else:
                        ...
                        if input_node_item is None or output_node_item is None:
                            should_remove = True
                            logger.debug("Orphaned pipe: port has no parent node")
                        elif (
                            input_node_item not in valid_node_items
                            or output_node_item not in valid_node_items
                        ):
                            should_remove = True
                            logger.debug("Orphaned pipe: parent node not in scene")
            ...
            if pipes_to_remove:
                ...
                logger.info(
                    f"Cleaned up {len(pipes_to_remove)} orphaned connection pipe(s)"
                )
        except Exception as e:
    ...
    def _get_serialized_workflow_data(self) -> Optional[dict]:
        ...
        try:
            ...
            serialized_data = {
                ...
                if hasattr(workflow.metadata, "to_dict")
                else {},
                "nodes": {},
                "connections": [],
    ...
    def _load_workflow_to_graph(self, workflow: WorkflowSchema) -> None:
        ...
        logger.debug(
            f"Loaded unified node mapping with {len(node_type_mapping)} node types"
        )
        ...
        for node_id, node_data in workflow.nodes.items():
            ...
            for widget_name, widget in widgets.items():
                if widget_name in config:
                    try:
                        ...
                        logger.debug(
                            f"Restored widget {node_id}.{widget_name} = '{config[widget_name]}'"
                        )
                    except Exception as e:
                        logger.warning(
            ...
            if pos and "x" in pos and "y" in pos:
                logger.debug(
                    f"Restoring position for {node_id}: ({pos['x']}, {pos['y']})"
                )
    ...
    def _on_import_workflow_json(self, json_text: str) -> None:
        ...
        try:
            ...
            self._main_window.statusBar().showMessage(
                f"Error pasting workflow: {str(e)}", 5000
            )
    ...
    def _on_export_selected(self, file_path: str) -> None:
        ...
        try:
            ...
            for visual_node in selected_nodes:
                ...
                node_data = {
                    "node_id": node_id,
                    "node_type": casare_node.node_type
                    if casare_node
                    else visual_node.type_,
            ...
            json_bytes = orjson.dumps(
                export_data, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS
            )
            Path(file_path).write_bytes(json_bytes)

            self._main_window.statusBar().showMessage(
                f"Exported {len(selected_ids)} nodes to {Path(file_path).name}", 5000
            )
            ...
            self._main_window.statusBar().showMessage(
                f"Error exporting nodes: {str(e)}", 5000
            )
    ...
    def _load_workflow_to_graph_merge(self, workflow_data: dict) -> None:
        ...
        logger.debug(
            f"Imported workflow with {len(workflow_data.get('nodes', {}))} nodes"
        )
    ...
    def _create_visual_node_from_data(self, graph, node_data: dict, factory):
        ...
        if not identifier:
            logger.warning(
                f"Unknown node type '{node_type}' - cannot create visual node"
            )
            return None

        try:
            visual_node = graph.create_node(
                identifier, pos=[pos.get("x", 0), pos.get("y", 0)]
            )
            ...
            if visual_node:
                ...
                if casare_class:
                    try:
                        ...
                        logger.error(
                            f"Failed to create CasareRPA node for {node_id}: {e}"
                        )
                    ...
                    for widget_name, value in config.items():
                        if widget_name in visual_node._widgets:
                            try:
                                ...
                                logger.debug(
                                    f"Could not restore widget {widget_name}: {e}"
                                )
            ...
            try:
                ...
                for visual_node in self._node_graph.graph.all_nodes():
                    ...
                    if visual_node_id:
                        ...
                        logger.info(
                            f"Visual node map: {visual_node_id} -> {visual_node.name()}"
                        )
                    else:
                        logger.warning(
                            f"Visual node {visual_node.name()} has no node_id property"
                        )
                ...
                for node_id, node in workflow.nodes.items():
                    ...
                    if node_id in visual_node_map:
                        ...
                        logger.info(
                            f"Saving {node_id}: pos=({pos[0]}, {pos[1]}), name='{visual_node.name()}'"
                        )
                    else:
    ...
    def _on_project_opened(self, project: Project) -> None:
        ...
        self._main_window.statusBar().showMessage(
            f"Project opened: {project.name}", 3000
        )
    ...
    def _on_scenario_opened(self, project: Project, scenario: Scenario) -> None:
        ...
        """
        logger.info(
            f"Scenario opened: {scenario.name} ({scenario.id}) in project {project.name}"
        )
    ...
    def _ensure_all_nodes_have_casare_nodes(self) -> bool:
        ...
        for visual_node in graph.all_nodes():
            if hasattr(visual_node, "ensure_casare_node"):
                ...
                if not casare_node:
                    logger.error(
                        f"Failed to create CasareRPA node for {visual_node.name()}"
                    )
                    all_valid = False
            else:
                logger.warning(
                    f"Node {visual_node.name()} doesn't support ensure_casare_node"
                )
    ...
    def _sync_visual_properties_to_node(self, visual_node, casare_node) -> None:
        ...
        if hasattr(visual_node, "paired_start_id") and hasattr(
            casare_node, "set_paired_start"
        ):
            paired_id = visual_node.get_property("paired_start_id") or getattr(
                visual_node, "paired_start_id", ""
            )
            logger.info(
                f"Loop end node {casare_node.node_id}: stored paired_id = '{paired_id}'"
            )
    ...
    def _detect_loop_start_pairing(self, loop_end_node) -> Optional[str]:
        ...
        try:
            ...
            end_type = (
                loop_end_node.NODE_NAME if hasattr(loop_end_node, "NODE_NAME") else ""
            )
            if "For" in end_type:
                ...
                for input_port in current.input_ports():
                    ...
                    for port in connected_ports:
                        ...
                        node_name = (
                            ...
                            if hasattr(source_node, "NODE_NAME")
                            else ""
                        )
                        if node_name == start_type and port.name() == "body":
                            ...
                            if start_node_id:
                                logger.info(
                                    f"Auto-detected loop pairing: {start_node_id} -> {loop_end_node.get_property('node_id')}"
                                )
        ...
        logger.warning(
            f"Loop pairing detection failed for {loop_end_node.NODE_NAME if hasattr(loop_end_node, 'NODE_NAME') else 'unknown'}"
        )
        return None
    ...
    def _load_project_variables_to_panel(
        self, manager: "ProjectManager", project_context: Optional["ProjectContext"]
    ) -> None:
        """
        ...
        try:
            ...
            if added_count > 0:
                ...
                logger.info(
                    f"Loaded {added_count} global/project variables into Variables Tab"
                )
    ...
    def _create_workflow_from_graph(self) -> WorkflowSchema:
        ...
        if not self._ensure_all_nodes_have_casare_nodes():
            logger.error(
                "Some nodes don't have CasareRPA nodes - workflow may be incomplete"
            )

        # Create workflow metadata
        metadata = WorkflowMetadata(
            name="Current Workflow", description="Workflow from visual editor"
        )
        ...
        for visual_node in graph.all_nodes():
            ...
            if casare_node:
                ...
                logger.warning(
                    f"Skipping node {visual_node.name()} - no CasareRPA node"
                )
        ...
        try:
            ...
            if not self._workflow_runner.subgraph_valid:
                ...
                QMessageBox.warning(
                    self._main_window,
                    "Cannot Run To Node",
                    "Target node is not reachable from StartNode.\n\n"
            ...
            async def run_and_cleanup():
                try:
                    ...
                    if runner.target_reached:
                        self._main_window.statusBar().showMessage(
                            "Reached target node - paused for inspection. "
                            "Press F8 to continue or F7 to stop.",
                            0,
                        )
    ...
    def _on_run_single_node(self, node_id: str) -> None:
        ...
        try:
            ...
            if not visual_node or not casare_node:
                ...
                self._main_window.statusBar().showMessage("Node not found", 3000)
                self._main_window.action_run_single_node.setEnabled(True)
                return
            ...
            async def execute_single_node():
                ...
                try:
                    ...
                    if success:
                        ...
                        logger.info(
                            f"Node {node_id} completed in {execution_time:.2f}s"
                        )
                        self._main_window.statusBar().showMessage(
    ...
    def _check_browser_launch(self, event) -> None:
        ...
        if context and context.active_page:
            ...
            is_new_page = (
                not self._selector_integration._active_page
                or self._selector_integration._active_page != context.active_page
            )
            ...
            if is_new_page:
                ...
                async def init_selector():
                    try:
                        await self._selector_integration.initialize_for_page(
                            context.active_page
                        )
    ...
    def _on_node_completed(self, event) -> None:
        ...
        if target_reached:
            ...
            self._main_window.action_run_single_node.setEnabled(True)
            self._main_window.statusBar().showMessage(
                "Target node reached - paused. Press F4 to re-run, F5 to run this node, F8 to continue, F7 to stop.",
                0,
            )
            return
        ...
        self._main_window.statusBar().showMessage(
            f"Workflow progress: {progress:.1f}%", 0
        )
    ...
    def _on_workflow_completed(self, event) -> None:
        ...
        logger.info(
            f"Workflow completed in {duration:.2f}s ({executed_nodes}/{total_nodes} nodes)"
        )
        self._main_window.statusBar().showMessage(
    ...
    def _on_start_selector_picking(self) -> None:
        ...
        async def start_picking():
            try:
                await self._selector_integration.start_picking(
                    target_node, target_property
                )
                self._main_window.statusBar().showMessage(
                    "Selector mode active - Click an element in the browser", 0
                )
    ...
    def _on_selector_picked(self, selector_value: str, selector_type: str) -> None:
        ...
        self._main_window.statusBar().showMessage(
            f"Selector picked: {selector_type}", 3000
        )
    ...
    def _on_recording_complete(self, actions: list) -> None:
        ...
        if dialog.exec():
            ...
            self._main_window.statusBar().showMessage(
                f"Generated workflow from {len(edited_actions)} actions", 5000
            )
        else:
            ...
            try:
                ...
                recorded_action = RecordedAction(
                    ...
                    timestamp=datetime.fromtimestamp(
                        action_dict.get("timestamp", 0) / 1000
                    ),
        ...
        if created_nodes and nodes:
            try:
                ...
                if nodes_with_outputs:
                    ...
                    logger.info(
                        f"Connected existing workflow: {last_node.name()} -> {first_new_node.name()}"
                    )
            except Exception as e:
    ...
    def _on_workflow_stopped(self, event) -> None:
        ...
        self._main_window.statusBar().showMessage(
            f"Workflow stopped ({executed_nodes}/{total_nodes} nodes)", 5000
        )
