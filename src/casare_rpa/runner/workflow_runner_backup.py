...
class WorkflowRunner:
    ...
    def _calculate_subgraph(self, target_node_id: NodeId) -> None:
        ...
        calculator = SubgraphCalculator(self.workflow.nodes, self.workflow.connections)

        # Check if target is reachable
        if not calculator.is_reachable(start_node.node_id, target_node_id):
            logger.error(
                f"Target node {target_node_id} is not reachable from StartNode"
            )
            return

        # Calculate the subgraph
        self._subgraph_nodes = calculator.calculate_subgraph(
            start_node.node_id, target_node_id
        )
        ...
        try:
            ...
            for attempt in range(1, self.retry_config.max_attempts + 1):
                try:
                    ...
                    self.retry_stats.record_attempt(
                        ...
                        retry_delay=0
                        if attempt == 1
                        else self.retry_config.get_delay(attempt - 1),
                    )
                    ...
                    self.retry_stats.record_attempt(
                        success=False,
                        retry_delay=self.retry_config.get_delay(attempt)
                        if attempt > 1
                        else 0,
                    )
            ...
            if result and result.get("success", False):
                ...
                error_msg = (
                    ...
                    if result
                    else str(last_exception or "No result")
                )
                self._emit_event(
                    EventType.NODE_ERROR,
                    {
    ...
    async def _execute_workflow_sequential(self, start_node: BaseNode) -> None:
        ...
        def _handle_try_error(
            context: ExecutionContext, error_msg: str, error_type: str
        ) -> Optional[str]:
        ...
        def _find_loop_body_nodes(loop_node_id: str) -> Set[str]:
            ...
            for conn in self.workflow.connections:
                if conn.source_node == loop_node_id and conn.source_port in (
                    "loop_body",
                    "body",
                ):
                    to_explore.append(conn.target_node)
        ...
        while nodes_to_execute and not self._stop_requested:
            ...
            is_loop_node = current_node.__class__.__name__ in [
                "ForLoopStartNode",
                "WhileLoopStartNode",
            ]
            ...
            if (
                current_node.node_id in self.executed_nodes
                and not is_loop_node
                and not is_in_active_loop
            ):
                continue
            ...
            if not success:
                ...
                if in_try_block:
                    ...
                    error_type = (
                        type(result.get("error", Exception())).__name__
                        if result
                        else "Exception"
                    )
                    logger.info(
                        f"Error in try block at {current_node.node_id}: {error_msg}"
                    )
                    ...
                    # Stop on error
                    logger.warning(
                        f"Stopping workflow due to node error: {current_node.node_id}"
                    )
                    break
            ...
            if control_flow == "break":
                ...
                if loop_node_id:
                    ...
                    nodes_to_execute = [
                        n for n in nodes_to_execute if n.node_id not in loop_body_nodes
                    ]
                    # Route to completed output of loop node
                    for conn in self.workflow.connections:
                        if (
                            conn.source_node == loop_node_id
                            and conn.source_port == "completed"
                        ):
                            if conn.target_node in self.workflow.nodes:
                                nodes_to_execute.insert(
                                    0, self.workflow.nodes[conn.target_node]
                                )
                                logger.debug(
                                    f"Break: routing to {loop_node_id}.completed -> {conn.target_node}"
                                )
                continue
            ...
            if result and "next_nodes" in result:
                ...
                if is_loop_node and (
                    "loop_body" in next_port_names or "body" in next_port_names
                ):
                    ...
                    logger.debug(
                        f"Entered loop {current_node.node_id}, body nodes: {loop_body_nodes}"
                    )
                ...
                if is_loop_node and "completed" in next_port_names:
                    ...
                    active_loops = [
                        ...
                        if l["loop_node_id"] != current_node.node_id
                    ]
                ...
                if is_try_node and "try_body" in next_port_names:
                    ...
                    logger.debug(
                        f"Entered try block {current_node.node_id}, body nodes: {try_body_nodes}"
                    )

                # Check if try block is completing (success or catch)
                if is_try_node and (
                    "success" in next_port_names or "catch" in next_port_names
                ):
                    # Remove this try block from active if present
                    active_try_blocks = [
                        t
                        for t in active_try_blocks
                        if t["try_node_id"] != current_node.node_id
                    ]
                ...
                for port_name in next_port_names:
                    ...
                    for connection in self.workflow.connections:
                        if (
                            connection.source_node == current_node.node_id
                            ...
                            if target_node_id in self.workflow.nodes:
                                next_nodes.append(self.workflow.nodes[target_node_id])
                                logger.debug(
                                    f"Dynamic routing: {current_node.node_id}.{port_name} -> {target_node_id}"
                                )
                ...
                if not nodes_to_execute and active_try_blocks:
                    ...
                    for try_info in reversed(active_try_blocks):
                        if current_node.node_id in try_info["try_body_nodes"]:
                            ...
                            pending_body_nodes = (
                                try_info["try_body_nodes"] - self.executed_nodes
                            )
                            pending_body_nodes.discard(current_node.node_id)
                            if not pending_body_nodes:
                                ...
                                logger.debug(
                                    f"Try body completed successfully, routing to {try_node_id} for success"
                                )
                            break
            else:
    ...
    async def _execute_workflow_parallel(self, start_node: BaseNode) -> None:
        ...
        if not success and not self.continue_on_error:
            logger.warning("Stopping workflow due to start node error")
            return
        ...
        while not self._stop_requested:
            ...
            ready_node_ids = [
                ...
                if nid not in self.executed_nodes
                or self.workflow.nodes[nid].__class__.__name__
                in ["ForLoopStartNode", "WhileLoopStartNode"]
            ]
            ...
            for node in sequential_nodes:
                ...
                if not success and not self.continue_on_error:
                    logger.warning(
                        f"Stopping workflow due to node error: {node.node_id}"
                    )
                    return
                ...
                if control_flow in ("break", "continue"):
                    logger.info(
                        f"{control_flow.capitalize()} signal from {node.node_id}"
                    )
    ...
    async def _execute_nodes_parallel(self, nodes: List[BaseNode]) -> None:
        ...
        logger.info(
            f"Executing {len(nodes)} nodes in parallel: {[n.node_id for n in nodes]}"
        )
        ...
        async def execute_single(
            node: BaseNode,
        ) -> Tuple[NodeId, bool, Optional[Dict[str, Any]]]:
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                if not success and not self.continue_on_error:
                    logger.warning(
                        f"Stopping parallel execution due to node error: {node_id}"
                    )
                    self._stop_requested = True
                    break
