...
Rules are modularized in `.agent/rules/`. All `.md` files define AI agent behavior.
...
See `.agent/rules/10-node-workflow.md` for full protocol.
...
2. Rules auto-load from `.agent/rules/`
3. Follow 5-phase workflow: RESEARCH -> PLAN -> EXECUTE -> VALIDATE -> DOCS

## Index Files

Each major module has `_index.md` for AI navigation:
